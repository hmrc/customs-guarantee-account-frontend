...
case class GuaranteeAccountTransactionsViewModel(
  transactions: Seq[GuaranteeAccountTransaction],
  ...
  val itemsGroupedByDate: Seq[GuaranteeAccountTransactionsByDate] = transactions
    .groupBy(_.guaranteeTransaction.date)
    .map(v => GuaranteeAccountTransactionsByDate(v._1, v._2))
    .toSeq
    .sortBy(_.date)
    .reverse
