...
case class GuaranteeAccountViewModel(account: GuaranteeAccount, currentDateTime: LocalDateTime)(implicit
  messages: Messages
) {

  val balanceAvailable: Option[String] =
    account.balances.map(bal => Formatters.formatCurrencyAmount0dp(bal.AvailableGuaranteeBalance))
