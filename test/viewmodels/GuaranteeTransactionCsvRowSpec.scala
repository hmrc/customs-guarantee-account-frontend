...
class GuaranteeTransactionCsvRowSpec extends SpecBase {
  ...
  "GuaranteeTransactionCsvRow" should {
    ...
    "generate detail rows for each tax type group" in {
      val multipleTaxTypeGroups = domainTransaction.copy(dueDates =
        Seq(
          DueDate(
            dueDate = "2020-07-28",
            reasonForSecurity = Some("T24"),
            amounts = amt,
            taxTypeGroups = Seq(ttg1, ttg2)
          )
        )
      )
    ...
    "generate a set of detail rows per security reason" in {
      val multipleSecurityReasons = domainTransaction.copy(dueDates =
        Seq(
          DueDate(
            dueDate = "2020-07-28",
            reasonForSecurity = Some("T24"),
          ...
          DueDate(
            dueDate = "2020-07-29",
            reasonForSecurity = Some("T25"),
            amounts = amt,
            taxTypeGroups = Seq(ttg2, ttg1)
          )
        )
      )

      multipleSecurityReasons.toReportLayout must be(
        Seq(
          GuaranteeTransactionCsvRow(
            date = Some("2020-03-01"),
            movementReferenceNumber = Some("mrn0"),
