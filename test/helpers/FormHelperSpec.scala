  ...
  "updateFormErrorKeyForStartAndEndDate" must {
    "append .month in the FormError key when key is either start or end and error msg key is " +
      ...
      "invalidStartMonthKey and invalidStartYearKey" in {
        ...
        FormHelper.updateFormErrorKeyForStartAndEndDate()(
          startKey,
          "cf.form.error.start.year.date-number-invalid"
        ) shouldBe s"$startKey.month"

        FormHelper.updateFormErrorKeyForStartAndEndDate()(
          endKey,
          "cf.form.error.end.date-too-far-in-past"
        ) shouldBe s"$endKey.month"
        ...
        FormHelper.updateFormErrorKeyForStartAndEndDate()(
          endKey,
          "cf.form.error.end.month.date-number-invalid"
        ) shouldBe s"$endKey.month"

        FormHelper.updateFormErrorKeyForStartAndEndDate()(
          endKey,
          "cf.form.error.end.year.date-number-invalid"
        ) shouldBe s"$endKey.month"
      }
